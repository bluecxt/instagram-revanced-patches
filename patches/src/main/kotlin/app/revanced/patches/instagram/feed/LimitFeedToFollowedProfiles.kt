...
val limitFeedToFollowedProfilesPatch = bytecodePatch(
    ...
    apply {
         ...
         *
         * The map field also identifies the feed request class itself, which its toString() debug
         * string cannot do unambiguously.
         */
        val headerMapField = mainFeedHeaderMapFinderMethod.let { method ->
            method.getInstruction(
                method.indexOfFirstInstructionOrThrow { fieldReference?.type == "Ljava/util/Map;" },
            ).getReference<FieldReference>()!!
        }

        val mainFeedRequestHeaderFieldName = headerMapField.name

        classDefs.first { it.type == headerMapField.definingClass }.firstMethodDeclaratively {
            name("<init>")
        }.apply {
